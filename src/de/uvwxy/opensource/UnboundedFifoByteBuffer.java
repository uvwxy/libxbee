...
/**
 * UnboundedFifoByteBuffer is a very efficient buffer implementation. According
 * to performance testing, it exhibits a constant access time, but it also
 * outperforms ArrayList when used for the same purpose.
 * <p>
 * The removal order of an <code>UnboundedFifoByteBuffer</code> is based on the
 ...
 * to provide synchronized access to your <code>UnboundedFifoByteBuffer</code>:
 * 
 * <pre>
...
public class UnboundedFifoByteBuffer {
	...
	/**
	 * Constructs an UnboundedFifoByteBuffer with the default number of
	 * elements. It is exactly the same as performing the following:
	 * 
	 * <pre>
	...
	/**
	 * Constructs an UnboundedFifoByteBuffer with the specified number of
	 * elements. The integer must be a positive integer.
	 * 
	 * @param initialSize
	...
	public boolean add(final byte b) {
		// Log.i("FIFO", "Added " + Integer.toHexString(b & 0xff) + "/" +
		// (char)b);
